...
def _set_transformers_logging() -> None:
    if os.getenv("GENAIPLAYGROUND_VERBOSITY", "INFO") in ["DEBUG", "INFO"]:
...
def _set_env_vars() -> None:
    if is_torch_npu_available():
        ...
        # avoid use fork method on NPU devices, see https://github.com/genai-playground/genai-playground/issues/7447
...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_CLS:
    ...
    if not model_args.use_kt and training_args.parallel_mode == ParallelMode.NOT_DISTRIBUTED:
        raise ValueError("Please launch distributed training with `genaiplayground-cli` or `torchrun`.")
