...
class Runner:
    ...
    def _launch(self, data: dict["Component", Any], do_train: bool) -> Generator[dict["Component", Any], None, None]:
        ...
        if error:
            ...
            save_args(os.path.join(args["output_dir"], PLAYGROUNDBOARD_CONFIG), self._build_config_dict(data))

            env = deepcopy(os.environ)
            env["PLAYGROUNDBOARD_ENABLED"] = "1"
            env["PLAYGROUNDBOARD_WORKDIR"] = args["output_dir"]
            if args.get("deepspeed", None) is not None:
                env["FORCE_TORCHRUN"] = "1"

            # NOTE: DO NOT USE shell=True to avoid security risk
            self.trainer = Popen(["genaiplayground-cli", "train", save_cmd(args)], env=env, stderr=PIPE, text=True)
    ...
    def check_output_dir(self, lang: str, model_name: str, finetuning_type: str, output_dir: str):
        ...
        if model_name and output_dir and os.path.isdir(get_save_dir(model_name, finetuning_type, output_dir)):
            ...
            config_dict = load_args(
                os.path.join(output_dir, PLAYGROUNDBOARD_CONFIG)
            )  # load playground board config
            for elem_id, value in config_dict.items():
                output_dict[self.manager.get_elem_by_id(elem_id)] = value
